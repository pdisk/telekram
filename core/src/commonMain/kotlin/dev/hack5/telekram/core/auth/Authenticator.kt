...
internal fun step4(
    ...
    val dataWithHash = pqInnerDataRepr.sha1().bytes + pqInnerDataRepr + randomData
...
internal fun step5(
        ...
        is ServerDHParamsFailObject -> {
            ...
            val newNonceHash = Int128Object.fromTlRepr(
                newNonce.asTlObject128().toTlRepr()
                    .toByteArray().sha1().bytes.sliceArray(4 until 20).toIntArray()
            )!!.second.native
    ...
    val checksum = decryptedAnswer.sliceArray(20 until answer!!.first * Int.SIZE_BYTES + 20).sha1().bytes
...
internal fun step6(
    ...
    val clientDHParamsHashed = clientDHParamsRepr.sha1().bytes + clientDHParamsRepr
...
internal fun validateNewNonceHash(newNonceHash: BigInteger, i: Int, newNonce: BigInteger, authKey: AuthKey) {
    val data =
        newNonce.asTlObject256().toTlRepr().toByteArray() + i.toByte() + authKey.auxHash.asTlObject().toTlRepr()
            .toByteArray()
    val hash = data.sha1().bytes
