...
package tk.hack5.telekram.generator
...
class NormalKtWriter(output: (String) -> Unit, private val entry: TLEntry, packageName: String) : KtWriter(output, packageName, entry.name) {
    ...
    private fun buildVector() {
        ...
        write("import tk.hack5.telekram.core.tl.*")
    ...
    fun writeImports() {
        ...
        when (entry.entryType) {
            EntryType.CONSTRUCTOR -> {
                ...
                entry.params.asSequence().map { it.type.toLowerCase().split("?", "<", ">") }
                    .flatten().distinct().map {
                        when (it) {
                            "int" -> listOf("import tk.hack5.telekram.core.tl.IntObject")
                            "long" -> listOf("import tk.hack5.telekram.core.tl.LongObject")
                            "double" -> listOf("import tk.hack5.telekram.core.tl.DoubleObject")
                            "string" -> listOf("import tk.hack5.telekram.core.tl.StringObject")
                            "bytes" -> listOf("import tk.hack5.telekram.core.tl.BytesObject")
                            "int128", "int256" -> {
                                listOf(
                                    "import tk.hack5.telekram.core.tl.asTlObject${it.takeLast(3)}",
                                    "import org.gciatto.kt.math.BigInteger",
                                    "import tk.hack5.telekram.core.tl.Int${it.takeLast(3)}Object"
                                )
                            }
                            else -> null
                        }
            ...
            EntryType.METHOD -> {
                write("import tk.hack5.telekram.core.tl.TLMethod")
                val type = entry.type.substringAfter("?").substringAfter("<").substringBefore(">")
                if (type in PRIMITIVE_TYPES)
                    write("import tk.hack5.telekram.core.tl.${type.capitalize()}Object")
        ...
        if (entry.params.any { it.type.split("?").last().toLowerCase() in PRIMITIVE_TYPES + Pair("bool", "") })
            write("import tk.hack5.telekram.core.tl.asTlObject")
    }
