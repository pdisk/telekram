...
class MapKtWriter(output: (String) -> Unit, private val data: TLData, packageName: String) : KtWriter(output, packageName) {
    override fun build() {
        writeHeader()
        write("import dev.hack5.telekram.core.tl.TLConstructor")
        write("import dev.hack5.telekram.core.tl.TLObject")
        write()
...
class NormalKtWriter(output: (String) -> Unit, private val entry: TLEntry, packageName: String) : KtWriter(output, packageName, entry.name) {
    ...
    private fun buildVector() {
        ...
        write("import dev.hack5.telekram.core.tl.*")
        ...
        write()
        write(
            "data class VectorObject<E : TLObject<*>>(private val list: Collection<E>, override val bare: Boolean) " +
    ...
    fun writeImports() {
        ...
        when (entry.entryType) {
            EntryType.CONSTRUCTOR -> {
                ...
                entry.params.asSequence().map { it.type.toLowerCase().split("?", "<", ">") }
                    .flatten().distinct().map {
                        when (it) {
                            "int" -> listOf("import dev.hack5.telekram.core.tl.IntObject")
                            "long" -> listOf("import dev.hack5.telekram.core.tl.LongObject")
                            "double" -> listOf("import dev.hack5.telekram.core.tl.DoubleObject")
                            "string" -> listOf("import dev.hack5.telekram.core.tl.StringObject")
                            "bytes" -> listOf("import dev.hack5.telekram.core.tl.BytesObject")
                            "int128", "int256" -> {
                                listOf(
                                    "import dev.hack5.telekram.core.tl.asTlObject${it.takeLast(3)}",
                                    "import org.gciatto.kt.math.BigInteger",
                                    "import dev.hack5.telekram.core.tl.Int${it.takeLast(3)}Object"
                                )
                            }
                            else -> null
                        }
            ...
            EntryType.METHOD -> {
                write("import dev.hack5.telekram.core.tl.TLMethod")
                val type = entry.type.substringAfter("?").substringAfter("<").substringBefore(">")
                if (type in PRIMITIVE_TYPES)
                    write("import dev.hack5.telekram.core.tl.${type.capitalize()}Object")
        ...
        if (entry.params.any { it.type.split("?").last().toLowerCase() in PRIMITIVE_TYPES + Pair("bool", "") })
            write("import dev.hack5.telekram.core.tl.asTlObject")
    }
    private fun writeClassDef() {
        ...
        entry.params.filter { it.type != "#" }.forEach {
            if (it.type == "bytes")
                write("if (!${it.name}.contentEquals(other.${it.name})) return false")
            else
...
class TypeKtWriter(output: (String) -> Unit, typeName: String, packageName: String, private val constructors: List<TLConstructor>) : KtWriter(output, packageName, typeName) {
    ...
    fun writeImports() {
        if (tlName == "Object")
            write("import dev.hack5.telekram.core.TlMappings")
        write("import dev.hack5.telekram.core.tl.TLObject")
        write("import dev.hack5.telekram.core.tl.TLTypeConstructor")
